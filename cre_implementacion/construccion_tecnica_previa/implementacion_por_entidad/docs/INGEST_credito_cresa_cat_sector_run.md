# Instrucciones de ejecución: `credito_cresa_cat_sector`

## 1. Objetivo

Ingerir `CREDITO_CRESA.dbo.cat_sector` en `dlh_cresa.bronze.credito_cresa_cat_sector` mediante el notebook metadata-driven, con controles de calidad, conteo de registros por tabla y alertas SMTP.

## 2. Configuración

YAML: `templates_ingenieria/config/ingestion/credito_cresa_cat_sector.yml`  
Job: `templates_ingenieria/config/ingestion/job/credito_cresa_cat_sector_job.json`  
JSON asociado: `templates_ingenieria/config/ingestion/JSON/credito_cresa_cat_sector.json`

La sentencia solicitada queda registrada en el YAML:

```sql
select id,nombre,es_activo,cat_zona_id,creado,actualizado,creado_por,actualizado_por,gestor_id  from CREDITO_CRESA.dbo.cat_sector nolock
```

Para ejecución SQL Server con table hint explícito, usar la variante equivalente `FROM CREDITO_CRESA.dbo.cat_sector WITH (NOLOCK)`.
...
```text
config_yml = analisis_caracterizacion/templates_ingenieria/config/ingestion/credito_cresa_cat_sector.yml
run_mode = test
...
SELECT COUNTIF(id IS NULL) AS nulos_id,
       ...
       COUNTIF(cat_zona_id IS NULL) AS nulos_cat_zona_id,
...
WHERE config_path LIKE '%cat_sector%'
...
- Validar dependencia referencial con `cat_zona` antes de la aprobación productiva.
